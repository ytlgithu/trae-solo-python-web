          ...
          {user ? (
            <>
              ...
              <div className="relative group">
                ...
                <div className="absolute right-0 top-full mt-2 w-48 glass rounded-2xl p-2 opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-300">
                  <div className="px-4 py-2 border-b border-border/50 mb-2">
                    <p className="text-sm font-bold">{user.username}</p>
                    <p className="text-xs text-muted">{user.role === 'ADMIN' ? '管理员' : '作者'}</p>
                  </div>
                  <Link to="/profile" className="block px-4 py-2 text-sm hover:bg-primary/10 hover:text-primary rounded-xl transition-colors">
                    个人中心
                  </Link>
                  <button 
                    onClick={logout}
